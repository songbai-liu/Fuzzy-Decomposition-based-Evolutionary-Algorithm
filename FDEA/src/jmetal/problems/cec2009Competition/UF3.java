...
public class UF3 extends Problem {
	...
	/**
	 * Constructor. Creates a default instance of problem CEC2009_UF3 (30
	 * decision variables)
	 * 
	 * @param solutionType
	...
	public void evaluate(Solution solution) throws JMException {
		...
		for (int j = 2; j <= numberOfVariables_; j++) {
			yj = x[j - 1]
					- Math.pow(x[0], 0.5 * (1.0 + 3.0 * (j - 2.0)
							/ (numberOfVariables_ - 2.0)));
		...
		solution.setObjective(0, x[0] + 2.0 * (4.0 * sum1 - 2.0 * prod1 + 2.0)
				/ (double) count1);
		solution.setObjective(1, 1.0 - Math.sqrt(x[0]) + 2.0
