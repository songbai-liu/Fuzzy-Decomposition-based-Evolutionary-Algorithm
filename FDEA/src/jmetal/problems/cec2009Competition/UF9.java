...
public class UF9 extends Problem {
	...
	/**
	 * Constructor. Creates a default instance of problem CEC2009_UF9 (30
	 * decision variables)
	 * 
	 * @param solutionType
	...
	public void evaluate(Solution solution) throws JMException {
		...
		for (int j = 3; j <= numberOfVariables_; j++) {
			yj = x[j - 1]
					- 2.0
					* x[1]
					* Math.sin(2.0 * Math.PI * x[0] + j * Math.PI
							/ numberOfVariables_);
			if (j % 3 == 1) {
				sum1 += yj * yj;
				count1++;
		...
		yj = (1.0 + epsilon_)
				* (1.0 - 4.0 * (2.0 * x[0] - 1.0) * (2.0 * x[0] - 1.0));
		if (yj < 0.0)
			yj = 0.0;

		solution.setObjective(0, 0.5 * (yj + 2 * x[0]) * x[1] + 2.0 * sum1
				/ (double) count1);
		solution.setObjective(1, 0.5 * (yj - 2 * x[0] + 2.0) * x[1] + 2.0
				* sum2 / (double) count2);
