...
/**
 * Class implementing shape functions for WFG benchmark Reference: Simon Huband,
 * Luigi Barone, Lyndon While, Phil Hingston A Scalable Multi-objective Test
 * Problem Toolkit. Evolutionary Multi-Criterion Optimization: Third
 * International Conference, EMO 2005. Proceedings, volume 3410 of Lecture Notes
 * in Computer Science
 */
public class Shapes {
	...
	/**
     * Calculate a convex2 shape
     */
    public float convex2(float[] x, int m) {
        float result = (float) 1.0;
        int M = x.length;
        ...
        if (m != 1)
            result *= (1 - Math.pow(Math.sin(x[M - m] * Math.PI * 0.5), 0.25));
     ...
     * Calculate a concave shape
     */
    public float concave2(float[] x, int m) {
        float result = (float) 1.0;
        int M = x.length;
        ...
        if (m != 1)
            result *= Math.pow(Math.cos(x[M - m] * Math.PI * 0.5), 0.25);
	...
	public float mixed(float[] x, int A, float alpha) {
		float tmp;
		tmp = (float) Math.cos((float) 2.0 * A * (float) Math.PI * x[0]
				+ (float) Math.PI * (float) 0.5);
	...
	public float disc(float[] x, int A, float alpha, float beta) {
		...
		return (float) 1.0 - (float) Math.pow(x[0], alpha)
				* (float) Math.pow(tmp, 2.0);
	} // disc
} // Shapes
