...
public class TwoPointsCrossover extends Crossover {
	...
	private static final List VALID_TYPES = Arrays
			.asList(PermutationSolutionType.class);
	...
	public Solution[] doCrossover(double probability, Solution parent1,
		...
		if (parent1.getType().getClass() == PermutationSolutionType.class) {
			if (PseudoRandom.randDouble() < probability) {
				...
				permutationLength = ((Permutation) parent1
						.getDecisionVariables()[0]).getLength();
				...
				offspring1Vector = ((Permutation) offspring[0]
						.getDecisionVariables()[0]).vector_;
				offspring2Vector = ((Permutation) offspring[1]
						.getDecisionVariables()[0]).vector_;
				...
				while (crosspoint2 == crosspoint1)
					crosspoint2 = PseudoRandom
							.randInt(0, permutationLength - 1);
	...
	public Object execute(Object object) throws JMException {
		...
		if (!(VALID_TYPES.contains(parents[0].getType().getClass()) && VALID_TYPES
			...
			Configuration.logger_
					.severe("TwoPointsCrossover.execute: the solutions "
		...
		if (parents.length < 2) {
			Configuration.logger_
					.severe("TwoPointsCrossover.execute: operator needs two "
							+ "parents");
			Class cls = java.lang.String.class;
			String name = cls.getName();
		...
		Solution[] offspring = doCrossover(crossoverProbability_, parents[0],
				parents[1]);
