 ...
 * @throws JMException 
  ...
  */
  ...
  public double distanceBetweenObjectivesShiftedTranslated(Solution solutionI, Solution solutionJ){                
	    ...
	    for (int nObj = 0; nObj < solutionI.numberOfObjectives();nObj++){
	    	diff = 0;
	    	if(solutionI.getUnitHypersphereObjective(nObj) < solutionJ.getUnitHypersphereObjective(nObj)){
	    		diff = solutionI.getUnitHypersphereObjective(nObj) - solutionJ.getUnitHypersphereObjective(nObj);
	    	}
	    	distance += diff*diff;                
	    } // for   
