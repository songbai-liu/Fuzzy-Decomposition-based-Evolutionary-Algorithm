...
public class RandomGenerator implements IRandomGenerator{
   ...
   * Constructor2
   */
  public RandomGenerator(double mySeed){
    this.seed = mySeed;
    this.randomize();
  } // RandomGenerator
  ...
  synchronized public int rndInt (int low, int high){
	...
	/**public double nextGaussianDouble(double mean, double std) {
...
The general contract of nextGaussian is that one double value,
 chosen from (approximately) the usual normal distribution with mean 0.0 and standard deviation 1.0,
 is pseudorandomly generated and returned. 
    ...
    **/
    ...
    public double nextGausssian(double mean, double std){
    	double s1 = this.nextStandardGaussian();
    	s1 = s1*std + mean;
    	return s1;
    }
