...
public class MOEA_C_N4 extends Algorithm {
	 ...
	 *            Problem to solve
	 */
	public MOEA_C_N4(Problem problem) {
		super(problem);
	...
	public void initLamda(){
		for(int i=0; i<populationSize_;i++){
			for(int j=0;j<problem_.getNumberOfObjectives();j++){
				lamda_[i][j] = population_.get(i).getNormalizedObjective(j);
			}
		}
	}
	...
	public void generateOffspringBasedNeighbor() throws JMException{
		...
		for (int i = 0; i < populationSize_; i++) {
			...
			Solution[] offSpring = (Solution[]) crossover_
					.execute(parents,delta);
			mutation_.execute(offSpring[0],delta);
	...
	public void environmentalSelection(){
		...
		distrectionMapingOnUnitplane(union_);
		
		population_.clear();
        List<SolutionSet> list = new <SolutionSet>ArrayList();
        if(generations_ > 1.0*maxGenerations_){
       ...
       /* for(int i=0;i<st[1].size();i++){
        ...
        if(generations_ < 1){
        	...
        	ck = 9;
        }else if(generations_ < 0.6*maxGenerations_){
        	ck = 8;
        }else if(generations_ < 0.7*maxGenerations_){
        	ck = 7;
        }else if(generations_ < 0.8*maxGenerations_){
        	ck = 6;
        }else if(generations_ < 0.9*maxGenerations_){
        	ck = 5;
        }else if(generations_ < 0.95*maxGenerations_){
        	ck = 4;
        }else{
        	ck = 4;
        }
        list = new HierarchicalClusteringInUnitHP(list,ck).clusteringAnalysis(populationSize_);	
	...
	public void distrectionMapingOnUnitplane(SolutionSet solutionSet){
		for(int i=0; i<solutionSet.size(); i++){
			Solution sol = solutionSet.get(i);
			for(int j=0; j<problem_.getNumberOfObjectives(); j++){
				double val = 0.0;
	...
	public void distrectionMapingOnUnitsphere(SolutionSet solutionSet){
		for(int i=0; i<solutionSet.size(); i++){
			Solution sol = solutionSet.get(i);
			for(int j=0; j<problem_.getNumberOfObjectives(); j++){
				double val = 0.0;
				val = sol.getNormalizedObjective(j)/sol.getDistanceToIdealPoint();
				sol.setUnitHyperplaneObjective(j, val);	
			}
		}
	}
	
	public void distrectionMapingOnDivertUnitsphere(SolutionSet solutionSet){
		for(int i=0; i<solutionSet.size(); i++){
			Solution sol = solutionSet.get(i);
			for(int j=0; j<problem_.getNumberOfObjectives(); j++){
				double val = 0.0;
				val = (sol.getNormalizedObjective(j)-1.0)/sol.getDistanceToNadirPoint();
				val = 1.0 + val;
				sol.setUnitHyperplaneObjective(j, val);	
			}
		}
    	...
    	for(int k=0; k<problem_.getNumberOfObjectives();k++){
    		double minClustering2Axis = 1.0e+30;
      		...
      		if(PseudoRandom.randDouble() <= 1.0){
      			double min1 = 1.0e+30;
      			int minIndex1 = -1;
      			Solution s1 = list.get(minClustering2AxisID).get(minSolution2AxisID);
      			for(int i=0;i<list.get(minClustering2AxisID).size();i++){
      				if(i!=minSolution2AxisID){
      					Solution s2 = list.get(minClustering2AxisID).get(i);
      					double angle1 =  computeAngle(s1,s2);
      					if(angle1 < min1){
      						min1 = angle1;
      						minIndex1 = i;
      					}
      			...
      			Solution s = null;
      			if(list.get(minClustering2AxisID).size() <= 1){
      				s = list.get(minClustering2AxisID).get(minSolution2AxisID);
      			}else{
      				if(s1.getDistanceToIdealPoint() < list.get(minClustering2AxisID).get(minIndex1).getDistanceToIdealPoint()){
          				s = s1;
          			}else{
          				s = list.get(minClustering2AxisID).get(minIndex1);
          			}
      			}
      			s.setLamda_(list.get(minClustering2AxisID).get(minSolution2AxisID));
      			population_.add(s);
          		list.remove(minClustering2AxisID);
      		}
    	}
    	//DominanceComparator dominanceCompare = new DominanceComparator();
  		double delta_ = 0.9;
  		...
  		while(it.hasNext()){
  			...
  			/*Ranking nodominatedRanking = new NondominatedRanking(sols);
  			SolutionSet front = nodominatedRanking.getSubfront(0);*/
  			double minFitness = 1.0e+30;
  			int minFitnessID = -1;
  			Solution sol1 = sols.getCentroid();
  			if(sols.size() == 0){
  				System.out.println("sol.size() == 0");
  				System.exit(0);
  			}
  			for(int j=0;j<sols.size();j++){
  				Solution sol2 = sols.get(j);
  				if(generations_ < maxGenerations_/2){
  			...
  			sols.get(minFitnessID).setLamda_(sols.getCentroid());
  	...
  	public void getNeighborhood() {
  		...
  		for (int i = 0; i < populationSize_; i++) {
  			...
  			for (int j = 0; j < populationSize_; j++) {
  				x[j] = computeAngle(population_.get(i).getLamda_(), population_.get(j).getLamda_());
  				idx[j] = j;
  			} // for
