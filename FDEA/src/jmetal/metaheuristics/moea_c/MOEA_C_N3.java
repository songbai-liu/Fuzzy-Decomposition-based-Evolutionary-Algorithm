...
public class MOEA_C_N3 extends Algorithm {
	 ...
	 *            Problem to solve
	 */
	public MOEA_C_N3(Problem problem) {
		super(problem);
	...
	public void environmentalSelection(){
		...
		if(rd <= 1.0){
			...
			type = 1;
			distrectionMapingOnUnitsphere(union_);
		}else{
			type = 2;
			distrectionMapingOnDivertUnitsphere(union_);
		}
		 ...
		 */
        list = new HierarchicalClusteringInUnitH(list,type).clusteringAnalysis(populationSize_);	
        /*
  ...
  /*  public void updateLamda(List<SolutionSet> list){
    	for(int i=0;i<list.size();i++){
    		for(int j=0; j< list.get(i).size();j++){
    			list.get(i).get(j).setDistrectionIndex(i);
    		}
    		for(int j=0;j<problem_.getNumberOfObjectives();j++){
    			lamda_[i][j] = list.get(i).getCentroid(0).getNormalizedObjective(j);
    		}
    	}
    }*/
