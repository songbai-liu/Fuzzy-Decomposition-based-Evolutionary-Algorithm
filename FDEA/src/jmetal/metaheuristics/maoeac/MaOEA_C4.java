...
public class MaOEA_C4 extends Algorithm{
	...
	public MaOEA_C4(Problem problem) {
		super(problem);
	...
	public void generateOffspringPopulation() throws JMException{
		...
		SolutionSet[] offspringSolutionSets = new PartitionalSolutionSet(population_,problem_.getNumberOfObjectives()).partitional();
		Solution[] gbests = new Solution[problem_.getNumberOfObjectives()];
		...
		for(int i=0;i<problem_.getNumberOfObjectives();i++ ){
			...
			//gbests[i+problem_.getNumberOfObjectives()] = offspringSolutionSets[i].get(1);
		...
		for(int i=0;i<problem_.getNumberOfObjectives();i++ ){
			...
			for(int j=0;j < offspringSolutionSets[i].size();j++){
				...
				double fitness0 = computeASFFitness(parents[0],parents[0]);
				double fitness1 = computeASFFitness(parents[1],parents[1]);
				if(fitness0 < fitness1){
					parents[1] = offspringSolutionSets[i].get(j);
				}else{
					parents[0] = offspringSolutionSets[i].get(j);
				}
				Solution[] offSpring = (Solution[]) crossover_
						.execute(parents);
				if(rd0 < 0.01){
					mutation_.execute(offSpring[0]);
				}else if(rd0 < 0.99 && rd0 > 0.01){
					...
					int rnd = PseudoRandom.randInt(0, offspringSolutionSets[i].size()/3);
	...
	public void environmentalSelection(){
		 ...
		 */
		SolutionSet[] solutionSets = new PartitionalSolutionSet(union_,problem_.getNumberOfObjectives()).partitional();
		for(int k=0;k<problem_.getNumberOfObjectives();k++){
			if(solutionSets[k].size() != 2*populationSize_/problem_.getNumberOfObjectives()){
				System.out.println("solutionSets["+k+"] = "+ solutionSets[k].size());
				System.exit(0);
			}
			SolutionSet st = getStSolutionSet(solutionSets[k],populationSize_/(problem_.getNumberOfObjectives()));
			...
			if(list.size() < populationSize_/(problem_.getNumberOfObjectives())){
			 ...
			 */
			list = new HierarchicalClustering1(list).clusteringAnalysis(populationSize_/(problem_.getNumberOfObjectives()));
			if(list.size() != populationSize_/(problem_.getNumberOfObjectives())){
   ...
   public void bestSolutionSelection(List<SolutionSet> list,int k){
	  double minClustering2Axis = 1.0e+30;
		...
		for(int i=0;i<list.size();i++){
			...
			double angle2 = Math.acos(sumValue/(sols.getCentroidVector().getDistanceToIdealPoint()*Math.sqrt(problem_.getNumberOfObjectives())));
		...
		for(int i=0;i<list.get(min2CenterLineId).size();i++){
			...
			double ang = Math.acos(Math.abs(sumValue/(sol.getDistanceToIdealPoint()*Math.sqrt(problem_.getNumberOfObjectives()))));
