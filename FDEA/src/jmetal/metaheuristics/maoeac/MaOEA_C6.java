...
public class MaOEA_C6 extends Algorithm{
	...
	public MaOEA_C6(Problem problem) {
		super(problem);
	...
	public void environmentalSelection(){
		 ...
		 */
		SolutionSet[] solutionSets = new PartitionalSolutionSet1(union_,problem_.getNumberOfObjectives()).partitional();
		for(int k=0;k<problem_.getNumberOfObjectives()+1;k++){
			if(solutionSets[k].size() != 2*populationSize_/(problem_.getNumberOfObjectives()+1)){
				System.out.println("solutionSets["+k+"] = "+ solutionSets[k].size());
				System.exit(0);
			}
			SolutionSet st = getStSolutionSet(solutionSets[k],populationSize_/((problem_.getNumberOfObjectives()+1)));
			...
			if(list.size() < populationSize_/(problem_.getNumberOfObjectives()+1)){
			 ...
			 */
			list = new HierarchicalClustering1(list).clusteringAnalysis(populationSize_/(problem_.getNumberOfObjectives()+1));
			if(list.size() != populationSize_/(problem_.getNumberOfObjectives()+1)){
   ...
   public void bestSolutionSelection(List<SolutionSet> list,int k){
	   if(k!=problem_.getNumberOfObjectives()){
