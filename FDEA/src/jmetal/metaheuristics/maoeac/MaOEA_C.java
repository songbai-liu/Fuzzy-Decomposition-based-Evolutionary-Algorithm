...
public class MaOEA_C extends Algorithm{
	...
	public MaOEA_C(Problem problem) {
		super(problem);
	...
	public double weightSumValue(int k,Solution solution2){
		...
		for(int i=0; i<problem_.getNumberOfObjectives();i++){
			if(i==k){
				value += (1.0/(2*Math.sqrt(problem_.getNumberOfObjectives()))+0.5)*solution2.getNormalizedObjective(i);
			}else{
				value += (1.0/(2*Math.sqrt(problem_.getNumberOfObjectives())))*solution2.getNormalizedObjective(i);
			}
		}
		return value;
	}
