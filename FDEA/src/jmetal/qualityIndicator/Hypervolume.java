...
public class Hypervolume {
	 ...
	 */
	int filterNondominatedSet(double[][] front, int numberofPoints, int numberofObjectives) {
		int i, j;
		int n;
	 ...
	 */
	double surfaceUnchangedTo (double[][] front, int numberofPoints, int numberofobjective){
		int i;
		double minValue, value;

		if (numberofPoints < 1)
			System.err.println("run-time error");
		minValue = front[0][numberofobjective];
		for (i = 1; i < numberofPoints; i++) {
			value = front[i][numberofobjective];
			if (value < minValue)
				minValue = value;
		}
		return minValue;
	...
	public double calculateHypervolume(double[][] front, int numberofPoints,
		...
		n = numberofPoints;
		while (n > 0) {
			int numberofNondominatedPoints;
			double tempVolume, tempDistance;

			numberofNondominatedPoints = filterNondominatedSet(front, n,
					numberofObjectives - 1);
			tempVolume = 0;
			if (numberofObjectives < 3) {
				if (numberofNondominatedPoints < 1)
					System.err.println("run-time error");

				tempVolume = front[0][0];
			} else
				tempVolume = calculateHypervolume(front, numberofNondominatedPoints,
						numberofObjectives - 1);
	...
	public double hypervolume(double[][] paretoFront,
		...
		maximumValues = utils_.getMaximumValues(paretoTrueFront,
				numberOfObjectives);
		minimumValues = utils_.getMinimumValues(paretoTrueFront,
				numberOfObjectives);

		// STEP 2. Get the normalized front
		normalizedFront = utils_.getNormalizedFront(paretoFront, maximumValues,
				minimumValues);
		...
		// code)
		return this.calculateHypervolume(invertedFront, invertedFront.length,
				numberOfObjectives);
	}// hypervolume
