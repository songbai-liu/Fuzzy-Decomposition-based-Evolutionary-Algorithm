...
*/	
	public wfghvCalculator4(double[][] paretoFront,int fun, int number) {
	    pf_ = paretoFront;
	    pfMatrix_ = null;
	    this.fun = fun;
	    this.number = number;
	...
	public double calculatewfghv() throws IOException{
	    ...
	    Solution referencePoint2 = new Solution( number);
		
	    for (int j=0;j<number;j++){
			if(fun == 6){//DTLZ1
				referencePoint1.setObjective(j,0.5);
				referencePoint2.setObjective(j,0.0);
				...
				if(j!=number-1){
					...
					referencePoint2.setObjective(number-1-j,Math.pow(Math.sin(Math.PI/8), j+1));
				}else{
					referencePoint1.setObjective(0,referencePoint1.getObjective(1));
					referencePoint2.setObjective(0,referencePoint2.getObjective(1));
				}
		...
		for (int j=0;j<sb.length;j++)
			for(int k=0;k<number;k++)
				sb[j][k] = (sb[j][k]-referencePoint2.getObjective(k))/(referencePoint1.getObjective(k)-referencePoint2.getObjective(k));
		...
		for (int j=0;j<sb.length;j++)
			for(int k=0;k<number;k++)
				if(sb[j][k]>referencePoint1.getObjective(k)){
					for(int s=0;s<number;s++){
						sb[j][s]  = referencePoint1.getObjective(k);
					}
					break;
				}
